...
export default function GuideFilters({ facets }: Props) {
  ...
  return (
    <div className="w-full bg-[var(--bg2)] px-7 mb-6">
      ...
      <div
        className={`transition-all duration-300 ease-in-out flex-col lg:max-h-[80vh] lg:opacity-100 lg:overflow-y-auto lg:no-scrollbar lg:pb-5 ${
          isMainOpen
            ? 'max-h-[2500px] opacity-100 flex pb-5 pt-4 overflow-hidden'
            : 'max-h-0 opacity-0 overflow-hidden hidden lg:flex lg:pt-2'
        }`}
      >
