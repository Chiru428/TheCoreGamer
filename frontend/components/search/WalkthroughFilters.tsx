...
interface Props {
  facets?: {
    ...
    games: { value: string; label: string; count?: number }[];
  };
}

export default function WalkthroughFilters({ facets }: Props) {
  ...
  const FILTER_CONFIG = [
    { id: 'gameSlug', label: 'Games', items: facets?.games || [] },
    { id: 'platform', label: 'Platforms', hideSearch: true, items: facets?.platforms || [] },
    { id: 'genre', label: 'Genres', items: facets?.genres || [] },
    { id: 'tag', label: 'Tags', items: facets?.tags || [] },
  ];
