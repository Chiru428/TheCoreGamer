...
export default function ReviewsListClient({ initialReviews, totalPages: initialTotalPages, sidebarChildren }: Props) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();
  const { hits, facets, isLoading, totalHits, totalPages: algoliaTotalPages, page: algoliaPage } = useAlgoliaArticles({ searchParams, contentType: 'REVIEW' });
