...
// --- Inline article search (editor-only, not the public-facing component) -----
...
export function ArticlePicker({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      if (res.success && res.data) setResults(res.data.filter(r => r.type === 'ARTICLE'));
      setLoading(false);
    }, 300);
        ...
        <div className="absolute z-50 w-full mt-1 rounded-lg shadow-2xl overflow-hidden border border-white/10"
          ...
          {results.map(r => (
            <button
              ...
              onClick={() => {
                onSelect({ slug: r.slug, title: r.title || r.slug, contentType: r.contentType || 'NEWS', imageUrl: r.imageUrl ?? undefined });
                setQuery('');
                setResults([]);
              }}
            >
              <span className="font-medium truncate">{r.title}</span>
              <span className="text-xs text-slate-400 truncate">/{r.slug} · {r.contentType}</span>
            </button>
          ))}
        </div>
      )}
