...
function AdminListiclesList() {
  ...
  const { data, mutate, isLoading } = useSWR(['admin-listicles', statusFilter, search, isAuthorRole, page], ([_, status, q, mine, p]) => fetchAdminPosts({ contentType: 'LISTICLE', status: status || undefined, search: q || undefined, sort: undefined, mine, page: Number(p), limit: 50 }));
  ...
  const handleDelete = async (slug: string) => {
    if (!confirm('Delete this listicle?')) return;
    setDeletingSlug(slug);
    const res = await deletePost(slug);
    setDeletingSlug(null);
    if (res.success) {
      addToast({ type: 'success', message: 'Listicle deleted' });
      mutate();
      await revalidatePublicPages();
    } else {
      addToast({ type: 'error', message: res.error || 'Failed' });
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold text-text-primary">Listicles</h1>
        <Link href="/admin/posts/new?type=LISTICLE"><Button size="sm" className="w-full sm:w-auto" icon={<Plus className="w-4 h-4" />}>New Listicle</Button></Link>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        ...
        {isLoading && (
          <div className="p-12 flex flex-col items-center justify-center text-text-muted">
            ...
            <p>Loading listicles...</p>
          </div>
        )}
        {!isLoading && articles.length === 0 && <div className="p-8 text-center text-text-muted">No listicles found</div>}
        <Pagination currentPage={page} totalPages={data?.pagination?.totalPages || 1} basePath="/admin/listicles" className="mt-6 mb-8" />
...
export default function AdminListiclesPage() {
  return (
