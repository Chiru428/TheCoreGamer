...
function AdminReviewsList() {
  ...
  const { data, mutate, isLoading } = useSWR(['admin-reviews', statusFilter, search, isAuthorRole, page], ([_, status, q, mine, p]) => fetchAdminPosts({ contentType: 'REVIEW', status: status || undefined, search: q || undefined, sort: undefined, mine, page: Number(p), limit: 50 }));
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <Link href="/admin/posts/new?type=REVIEW"><Button size="sm" className="w-full sm:w-auto" icon={<Plus className="w-4 h-4" />}>New Review</Button></Link>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full text-sm">
          <thead className="bg-bg-elevated">
            <tr>
              ...
              <th className="hidden sm:table-cell px-4 py-3 text-left text-text-muted font-medium">Score</th>
              ...
              {articles.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-bg-elevated/50">
                    ...
                    <td className="hidden sm:table-cell px-4 py-3 text-accent-green font-bold">{a.gameReview?.reviewScore ? `${Number(a.gameReview.reviewScore).toFixed(1)}/10` : '-'}</td>
                    ...
                    <td className="px-4 py-3 text-center text-text-muted">
                      <div className="flex items-center justify-center gap-1">
                        <Link href={`/reviews/${a.slug}`} className="p-1.5 rounded hover:bg-bg-elevated text-text-dim hover:text-text-primary"><Eye className="w-4 h-4" /></Link>
        ...
        {!isLoading && articles.length === 0 && <div className="p-8 text-center text-text-muted">No reviews found</div>}
        <Pagination currentPage={page} totalPages={data?.pagination?.totalPages || 1} basePath="/admin/reviews" className="mt-6 mb-8" />
