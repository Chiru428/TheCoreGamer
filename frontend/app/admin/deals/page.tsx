...
function AdminDealsList() {
  ...
  const { data, mutate, isLoading } = useSWR(['admin-deals', statusFilter, search, isAuthorRole, page], ([_, status, q, mine, p]) => fetchAdminPosts({ contentType: 'DEAL', status: status || undefined, search: q || undefined, sort: undefined, mine, page: Number(p), limit: 50 }));
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <Link href="/admin/posts/new?type=DEAL"><Button size="sm" className="w-full sm:w-auto" icon={<Plus className="w-4 h-4" />}>New Deal</Button></Link>
      </div>
                  ...
                  <tr key={a.id} className="hover:bg-bg-elevated/50">
                    ...
                    <td className="px-4 py-3 text-center text-text-muted">
                      <div className="flex items-center justify-center gap-1">
                        <Link href={`/deals/${a.slug}`} className="p-1.5 rounded hover:bg-bg-elevated text-text-dim hover:text-text-primary"><Eye className="w-4 h-4" /></Link>
