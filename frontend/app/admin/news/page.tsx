...
export default function AdminNewsPage() {
  ...
  const { data, mutate, isLoading } = useSWR(['admin-reviews', statusFilter, search, isAuthorRole], ([_, status, q, mine]) => fetchAdminPosts({ contentType: 'NEWS', status: status || undefined, search: q || undefined, sort: 'updated', mine }));
  ...
  const handleDelete = async (slug: string) => {
    ...
    if (res.success) {
      addToast({ type: 'success', message: 'News deleted' });
      mutate();
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <Link href="/admin/posts/new?type=NEWS"><Button size="sm" className="w-full sm:w-auto" icon={<Plus className="w-4 h-4" />}>New News</Button></Link>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        ...
        {!isLoading && articles.length === 0 && <div className="p-8 text-center text-text-muted">No news found</div>}
      </div>
    </div>
  );
}
