...
function AdminFeaturesList() {
  ...
  const { data, mutate, isLoading } = useSWR(['admin-reviews', statusFilter, search, isAuthorRole, page], ([_, status, q, mine, p]) => fetchAdminPosts({ contentType: 'FEATURE', status: status || undefined, search: q || undefined, sort: 'updated', mine, page: Number(p), limit: 50 }));
  ...
  const handleDelete = async (slug: string) => {
    ...
    if (res.success) {
      addToast({ type: 'success', message: 'Feature deleted' });
      mutate();
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <Link href="/admin/posts/new?type=FEATURE"><Button size="sm" className="w-full sm:w-auto" icon={<Plus className="w-4 h-4" />}>New Feature</Button></Link>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
                  ...
                  <tr key={a.id} className="hover:bg-bg-elevated/50">
                    ...
                    <td className="px-4 py-3 text-center text-text-muted">
                      <div className="flex items-center justify-center gap-1">
                        <Link href={`/features/${a.slug}`} className="p-1.5 rounded hover:bg-bg-elevated text-text-dim hover:text-text-primary"><Eye className="w-4 h-4" /></Link>
        ...
        {!isLoading && articles.length === 0 && <div className="p-8 text-center text-text-muted">No features found</div>}
        <Pagination currentPage={page} totalPages={data?.pagination?.totalPages || 1} basePath="/admin/features" className="mt-6 mb-8" />
