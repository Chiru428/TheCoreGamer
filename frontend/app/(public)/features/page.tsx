...
export const metadata = buildMeta({ title: 'Features', description: 'In-depth gaming features, deep dives and long reads from theCoreGamer', url: '/features' });
...
export default async function FeaturesPage({ searchParams }: Props) {
  ...
  const [mainRes, popularRes] = await Promise.all([
    fetchPosts({ page, sort, limit: 20, contentType: 'FEATURE', revalidate: 60 }),
    fetchPosts({ page: 1, limit: 8, sort: 'popular', contentType: 'FEATURE', revalidate: 60 }),
  ]);
  ...
  return (
    <>
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 lg:px-0 pt-6 md:pt-10 pb-8">
        <ContentTypeHeading title="FEATURES" />
        <div className="mb-4 md:mb-8 flex flex-wrap items-center justify-between gap-4">
          <CategorySearch contentType="FEATURE" placeholder="Search feature articles..." />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] xl:grid-cols-[900px_1fr] gap-6 md:gap-8 items-start min-h-screen">
          <div className="lg:border-r-2 lg:border-border lg:pr-8">
            {articles.length === 0 ? (
              <div className="text-center py-20 text-text-muted">No feature articles yet. Check back soon!</div>
            ) : (
              articles.map((a, i) => (
                <div key={a.id}>
