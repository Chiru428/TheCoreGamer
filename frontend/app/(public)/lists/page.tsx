...
export const metadata = buildMeta({ title: 'Lists', description: 'Top gaming lists, rankings and roundups from theCoreGamer', url: '/lists' });
...
export default async function ListsPage({ searchParams }: Props) {
  ...
  const [mainRes, popularRes] = await Promise.all([
    fetchPosts({ page, sort, limit: 20, contentType: 'LISTICLE', revalidate: 60 }),
    fetchPosts({ page: 1, limit: 8, sort: 'popular', contentType: 'LISTICLE', revalidate: 60 }),
  ]);
  ...
  return (
    <>
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 lg:px-0 pt-6 md:pt-10 pb-8">
        <ContentTypeHeading title="CORE PICKS" />
        <div className="mb-4 md:mb-8 flex flex-wrap items-center justify-between gap-4">
          <CategorySearch contentType="LISTICLE" placeholder="Search list articles..." />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] xl:grid-cols-[900px_1fr] gap-6 md:gap-8 items-start min-h-screen">
          <div className="lg:border-r-2 lg:border-border lg:pr-8">
            {articles.length === 0 ? (
              <div className="text-center py-20 text-text-muted">No lists yet. Check back soon!</div>
            ) : (
              articles.map((a, i) => (
                <div key={a.id}>
