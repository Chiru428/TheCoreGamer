...
export const metadata = buildMeta({ title: 'Opinions', description: 'Gaming opinions, editorials and hot takes from theCoreGamer team', url: '/opinions' });

interface Props { searchParams: Promise<Record<string, string | undefined>> }

export default async function OpinionsPage({ searchParams }: Props) {
  const params = await searchParams;
  const page = Number(params.page) || 1;
  const sort = params.sort;

  const [mainRes, popularRes] = await Promise.all([
    fetchPosts({ page, sort, limit: 20, contentType: 'OPINION', revalidate: 60 }),
    fetchPosts({ page: 1, limit: 8, sort: 'popular', contentType: 'OPINION', revalidate: 60 }),
  ]);
  ...
  return (
    <>
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 lg:px-0 pt-6 md:pt-10 pb-8">
        <ContentTypeHeading title="OPINIONS" />
        <div className="mb-4 md:mb-8 flex flex-wrap items-center justify-between gap-4">
          <CategorySearch contentType="OPINION" placeholder="Search opinion articles..." />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] xl:grid-cols-[900px_1fr] gap-6 md:gap-8 items-start min-h-screen">
          <div className="lg:border-r-2 lg:border-border lg:pr-8">
            {articles.length === 0 ? (
              <div className="text-center py-20 text-text-muted">No opinion pieces yet. Check back soon!</div>
            ) : (
              articles.map((a, i) => (
                <div key={a.id}>
          ...
          <aside className="order-first lg:order-none flex flex-col gap-4 pb-0 md:pb-8 md:gap-6 md:w-full self-stretch">
            <TrendingSidebarBox title="Popular Opinions" articles={popularArticles} />
