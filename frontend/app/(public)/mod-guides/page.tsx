import { fetchModGuides } from '@/lib/api';
...
export const metadata = buildMeta({ title: 'Mod Guides', description: 'Step-by-step modding guides for your favorite games', url: '/mod-guides' });

interface Props { searchParams: Promise<Record<string, string | undefined>> }

export default async function ModGuidesListingPage({ searchParams }: Props) {
  const params = await searchParams;
  const page = Number(params.page) || 1;
  const sort = params.sort;

  const [mainRes, popularRes] = await Promise.all([
    fetchModGuides({ page, sort, limit: 20, revalidate: 60 }),
    fetchModGuides({ page: 1, limit: 8, sort: 'popular', revalidate: 60 }),
  ]);
  ...
  return (
    <>
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 lg:px-0 pt-6 md:pt-10 pb-8">
        <ContentTypeHeading title="MOD GUIDES" />
        <div className="mb-4 md:mb-8 flex flex-wrap items-center justify-between gap-4">
          <CategorySearch contentType="MOD_GUIDE" placeholder="Search mod guides..." />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] xl:grid-cols-[900px_1fr] gap-6 md:gap-8 items-start min-h-screen">
        ...
        <div className="lg:border-r-2 lg:border-border lg:pr-8">
          {articles.length === 0 ? (
            <div className="text-center py-20 text-text-muted">No mod guides yet. Check back soon!</div>
          ) : (
            articles.map((a, i) => (
              <div key={a.id}>
