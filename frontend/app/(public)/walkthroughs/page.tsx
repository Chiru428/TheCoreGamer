import { fetchPosts, fetchWalkthroughFacets } from '@/lib/api';
import Pagination from '@/components/ui/Pagination';
import SortDropdown, { referenceSortOptions } from '@/components/ui/SortDropdown';
...
import WalkthroughFilters from '@/components/search/WalkthroughFilters';
import WalkthroughActiveFilters from '@/components/search/WalkthroughActiveFilters';
import ContentTypeHeading from '@/components/ui/ContentTypeHeading';

export const dynamic = 'force-dynamic';
export const metadata = buildMeta({
  title: 'Walkthroughs & Guides',
  description: 'Comprehensive, step-by-step game walkthroughs, boss guides, and item locations.',
  url: '/walkthroughs',
});

interface Props { searchParams: Promise<Record<string, string | undefined>> }

export default async function WalkthroughsListingPage({ searchParams }: Props) {
  ...
  const [mainRes, popularRes, facetsRes] = await Promise.all([
    fetchPosts({ contentType: 'WALKTHROUGH', page, sort, platform, genre, tag, gameSlug, limit: 20, revalidate: 60 }),
    fetchPosts({ contentType: 'WALKTHROUGH', page: 1, limit: 8, sort: 'popular', revalidate: 60 }),
    fetchWalkthroughFacets(300)
  ]);
  ...
  const facets = {
    platforms: (facetsData.platforms || []).map((p: any) => ({ ...p, label: p.label || p.value })),
    genres: (facetsData.genres || []).map((g: any) => ({ ...g, label: g.label || g.value })),
    tags: facetsData.tags || [],
    games: facetsData.games || [],
  };
  ...
  return (
    <>
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 lg:px-0 pt-6 md:pt-10 pb-8">
        <ContentTypeHeading title="WALKTHROUGHS" />
        <div className="mb-4 md:mb-8 flex flex-wrap items-center justify-between gap-4">
          <CategorySearch contentType="WALKTHROUGH" placeholder="Search walkthroughs..." />
          <SortDropdown options={referenceSortOptions} />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] xl:grid-cols-[900px_1fr] gap-6 md:gap-8 items-start min-h-screen">
        ...
        <div className="lg:border-r-2 lg:border-border lg:pr-8">
          <WalkthroughActiveFilters totalResults={mainRes.pagination?.total || 0} />
          
          {articles.length === 0 ? (
            <div className="text-center py-20 text-text-muted">No walkthroughs yet. Check back soon!</div>
          ) : (
            articles.map((a, i) => (
              <div key={a.id}>
        ...
        <aside className="order-first lg:order-none flex flex-col gap-4 pb-0 md:pb-8 md:gap-6 md:w-full self-stretch">
          <WalkthroughFilters facets={facets} />
